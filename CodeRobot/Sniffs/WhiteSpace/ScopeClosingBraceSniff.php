<?php
  /**
   * CodeRobot_Sniffs_Whitespace_ScopeClosingBraceSniff.
   *
   * Checks that the closing braces of scopes are aligned correctly.
   */
  class CodeRobot_Sniffs_WhiteSpace_ScopeClosingBraceSniff implements PHP_CodeSniffer_Sniff {
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr) {
      ...
      if(in_array($tokens[$stackPtr]['code'], array(T_CASE, T_DEFAULT)) === TRUE
        ...
        if($braceIndent !== ($startColumn)) {
          $error = 'Break statement indented incorrectly; expected ' . $startColumn .' spaces, found ' . ($braceIndent - 1);
          $phpcsFile->addError($error, $scopeEnd);
        }
      } else {
        if($braceIndent !== $startColumn) {
