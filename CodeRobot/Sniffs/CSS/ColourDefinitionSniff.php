 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class CodeRobot_Sniffs_CSS_ColourDefinitionSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($colour !== $expected) {
            $error = "CSS colors must be defined in uppercase; expected $expected but found $colour";
        ...
        if ($colour{1} === $colour{2} && $colour{3} === $colour{4} && $colour{5} === $colour{6}) {
            ...
            $error    = "CSS colors must use shorthand if available; expected $expected but found $colour";
            $phpcsFile->addError($error, $stackPtr);
        }
